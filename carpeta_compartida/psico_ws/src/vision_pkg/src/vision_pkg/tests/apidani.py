...
# 👉 VISIÓN: Añadimos el import de los mensajes de visión
...
# 👉 VISIÓN: Cliente para la prueba de visión
class VisionClient:
    """Cliente para la acción `vision` (vision_pkg/VisionAction)."""

    @staticmethod
    def run(cadena_usuario: str) -> float:
        """
        Envía la cadena del paciente al action server y devuelve la nota (0-10).
        """
        registrar(f"Visión: enviando cadena '{cadena_usuario}' al servidor...")
        cliente = actionlib.SimpleActionClient('vision', VisionAction)
        if not cliente.wait_for_server(timeout=rospy.Duration(5.0)):
            registrar("ERROR: Servidor 'vision' no disponible.")
            return -1.0

        objetivo = VisionGoal()
        objetivo.input_str = cadena_usuario
        cliente.send_goal(objetivo)
        cliente.wait_for_result(rospy.Duration(10.0))
        resultado = cliente.get_result()
        puntuacion = resultado.result if resultado else -1.0
        registrar(f"Visión: nota recibida = {puntuacion}")
        return puntuacion
...
def save_session_csv(sesion: Dict[str, Any], csv_path: Path = HISTORY_CSV):
    ...
    notas = {"memoria": None, "reflejos": None, "audicion": None, "audicion_p1": None, "audicion_p2": None, "vision": None}  # 👈 Añadido "vision"
...
def _seed_fake_session(nombre: str = "Prueba"):
    ...
    vision_nota = round(random.uniform(5.0, 10.0), 2)  # 👈 Añadido
    ...
    sesion = {
        ...
        "pruebas": [
            ...
            {"prueba": "vision",   "puntuacion": vision_nota, "hora": ts.strftime("%H:%M:%S")},  # 👈 Añadido
        ]
    }

    SESION.update({
        "ejecutando": False,
        "pruebas_completadas": ["memoria", "reflejos", "audicion", "vision"],  # 👈 Añadido
        "resultados": list(sesion["pruebas"]),
        "paciente": {"nombre": nombre},
        "total_pruebas": 4,  # 👈 Añadido
        "current": "",
    })
...
# 👉 VISIÓN: Añadimos "visión" a TEST_DISPLAY
TEST_DISPLAY = {
    "memoria": "Memoria",
    "reflejos": "Reflejos",
    "audicion": "Audición",
    "vision": "Visión",  # 👈 Añadido
}
...
def iniciar_pruebas():
    ...
    # 👉 VISIÓN: extraer la cadena de visión si está presente
    vision_cadena = datos.get("vision_cadena", "")
    ...
    def worker():
        for idx, id_prueba in enumerate(secuencia_pruebas, start=1):
            ...
            # 👉 VISIÓN: Añadimos el caso para "vision"
            elif id_prueba == "vision":
                puntuacion = VisionClient.run(vision_cadena)
                payload = {"prueba": id_prueba, "puntuacion": puntuacion, "hora": datetime.now().strftime("%H:%M:%S")}
            else:
